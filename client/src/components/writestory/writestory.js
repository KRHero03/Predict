...
class WriteStory extends React.Component {
  ...
  render() {
    ...
    const addStory = async () => {
      this.setState({
        isAddStoryClicked: true
      })
      const storyResponse = await axios.post('/api/add_story', {
            ...
            <Container className={classes.base}>
              ...
              <Grid container xs={12} justify="space-between">
                ...
                <Grid item xs={12} style={{ width: '100%', justifyContent: 'center', display: 'flex' }}>
                  {this.state.isAddStoryClicked ? (
                    <CircularProgress />
                  ) : (

                      <Button style={{ width: '100%', marginBottom: 30, marginTop: 30 }} onClick={addStory} color="primary" variant="contained">
                        ADD STORY
                      </Button>
                    )}
                </Grid>
...
export default connect(mapStateToProps,
  { setCurrentUser })(withStyles(styles)(WriteStory));